package com.chainsys.ebus.dao.impl;

import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.SQLException;

import com.chainsys.ebus.Connection.TestConnection;
import com.chainsys.ebus.Exception.DbException;
import com.chainsys.ebus.Exception.infoMessages;
import com.chainsys.ebus.dao.paymentDAO;
import com.chainsys.ebus.model.passengerInfo;

public class paymentDAOImpl implements paymentDAO {

	public void paymentSuccess(int bookingId) throws Exception {

		try (Connection con = TestConnection.connection();) {
			String sql = "update payment_status set paid_status='success' where booking_id=?";
			try (PreparedStatement pst = con.prepareStatement(sql);) {
				pst.setInt(1, bookingId);
				pst.executeUpdate();

				updateBookingStatus(bookingId);

			}
		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYMENTSUCCESS);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}
	}

	public void updateBookingStatus(int bookingId) throws Exception {

		try (Connection con = TestConnection.connection();) {

			String sql1 = "update passenger_details set booking_status='booked' where booking_id=?";
			try (PreparedStatement pst1 = con.prepareStatement(sql1);) {
				pst1.setInt(1, bookingId);
				pst1.executeUpdate();
				updateSeatAvailability(bookingId);

			}
		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYMENTSUCCESS);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}
	}

	public void updateSeatAvailability(int bookingId) throws Exception {

		try (Connection con = TestConnection.connection();) {

			String sql2 = "update seat_availability set available_seats=available_seats -(select no_of_tickets from passenger_details where booking_id= ?)where bus_id=(select bus_id from passenger_details where booking_id=?)  ";
			try (PreparedStatement pst2 = con.prepareStatement(sql2);) {
				pst2.setInt(1, bookingId);
				pst2.setInt(2, bookingId);
				pst2.executeUpdate();

			}
		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYMENTSUCCESS);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}

	}

	public void paymentFailure(int bookingId) throws Exception {
		try (Connection con = TestConnection.connection();) {
			String sql = "update payment_status set paid_status='failure' where booking_id=?";
			try (PreparedStatement pst = con.prepareStatement(sql);) {
				pst.setInt(1, bookingId);
				pst.executeUpdate();

				updateFailureBookingStatus(bookingId);

			}
		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYMENTFAILURE);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}

	}

	public void updateFailureBookingStatus(int bookingId) throws Exception {
		try (Connection con = TestConnection.connection();) {

			String sql2 = "update passenger_details set booking_status='cancelled' where booking_id=?";
			try (PreparedStatement pst1 = con.prepareStatement(sql2);) {
				pst1.setInt(1, bookingId);
				pst1.executeUpdate();

			}
		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYMENTFAILURE);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}

	}

	@Override
	public void cashPay(int bookingId) throws Exception {
		// TODO Auto-generated method stub
		try (Connection con = TestConnection.connection();) {
			String sql = "update payment_status set paid_status='COD' where booking_id=?";
			try (PreparedStatement pst = con.prepareStatement(sql);) {
				pst.setInt(1, bookingId);
				pst.executeUpdate();

				updateBookingStatusCash(bookingId);

			}
		}

		catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYCASH);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);
		}

	}

	public void updateBookingStatusCash(int bookingId) throws Exception {
		// TODO Auto-generated method stub
		try (Connection con = TestConnection.connection();) {
			String sql1 = "update passenger_details set booking_status='booked' where booking_id=?";
			try (PreparedStatement pst1 = con.prepareStatement(sql1);) {
				pst1.setInt(1, bookingId);
				pst1.executeUpdate();

				updateSeatAvailabilityCash(bookingId);
			}

		} catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYCASH);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);

		}

	}

	public void updateSeatAvailabilityCash(int bookingId) throws Exception {
		// TODO Auto-generated method stub
		try (Connection con = TestConnection.connection();) {

			String sql2 = "update seat_availability set available_seats=available_seats -(select no_of_tickets from passenger_details where booking_id= ?)where bus_id=(select bus_id from passenger_details where booking_id=?)  ";
			try (PreparedStatement pst2 = con.prepareStatement(sql2);) {
				pst2.setInt(1, bookingId);
				pst2.setInt(2, bookingId);
				pst2.executeUpdate();

			}
		}

		catch (SQLException e) {

			e.printStackTrace();
			throw new DbException(infoMessages.PAYCASH);

		} catch (Exception e) {

			e.printStackTrace();
			throw new DbException(infoMessages.CONNECTION);
		}

	}
}
